...
int * shortestPaths(Map g, LocationID start, int trans[], int numTrans) {
   // allocate arrays which, for a given location, keep track of the 
   // (shortest) number of edges in the path to it, and the location and
   // type of transposrt which lead to it in that path.
   int * distanceTo = malloc (g->nV * sizeof(int));
   
   // initialiase all values in the distanceTo array, 
   int i;
   for (i = 0; i < g->nV; i++) { distanceTo[i] = UNVISITED; }
 
   // create a new queue to perform BFS and add the start location 
   // to it
   Queue q = newQueue();
   QueueJoin(q, start);
   distanceTo[start] = 0;
 
   LocationID curr; VList neighbour;
   while (!QueueIsEmpty(q)) {
      // pop the latest location from the queue
      curr = QueueLeave (q);
      
      // iterate through the adjacency list for the current
      // location.
      for (neighbour = g->connections[curr]; neighbour != NULL; 
           neighbour = neighbour->next) {
         // for each neighbouring location that we havent already seen before, 
         // we'll first check if it's equal to dest, otherwise we'll add it to 
         // the queue to search later.
         if (distanceTo[neighbour->v] == UNVISITED) {
            int isValidTransport = 0;
            for (i = 0; i < numTrans; i++) {
               if (neighbour->type == trans[i]) {
                  isValidTransport = 1;
                  break;
               }
            }
            ...
            if (isValidTransport) {
               // the number of edges that it took to get to v is the number that it
               // took to get to curr + 1
               distanceTo[neighbour->v] = distanceTo[curr] + 1;
    
               QueueJoin (q, neighbour->v);
            }
         }
      }
   }
...
LocationID * getConnections(Map g, LocationID l, TransportID t, int pathLength, int * numConnections) {
   int * distanceTo = malloc (g->nV * sizeof(int));
   ...
   *numConnections = 1;
   ...
   while (!QueueIsEmpty(q)) {
      curr = QueueLeave(q);
      if (distanceTo[curr] >= pathLength) { break; }

      for (neighbour = g->connections[curr]; 
            neighbour != NULL; neighbour = neighbour->next) {
            if (distanceTo[neighbour->v] == UNVISITED && neighbour->type == t) {
               (*numConnections)++;
               distanceTo[neighbour->v] = distanceTo[curr] + 1;
               QueueJoin (q, neighbour->v);
            }
   ...
   LocationID * connections = malloc (*numConnections * sizeof(LocationID));
   int j = 0;
   for (i = 0; i < g->nV; i++) { 
      if (distanceTo[i] != UNVISITED) {
         connections[j] = i; 
         j++;
      }
   }
